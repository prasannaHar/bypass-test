...
class TestHygieneVsIssue:
    ...
    ou_ids = generic_object.env["set_ous"]
    across_type = generic_object.get_aggregration_fields()

    # across_type = generic_object.get_aggregration_fields(ou_id=ou_ids[0])
    # breakpoint()

    # across_type = {'custom_field-customfield_10003-Epic Name'}

    @pytest.mark.run(order=1)
    @pytest.mark.dataaccuracy
    @pytest.mark.parametrize("across_type", across_type)
    def test_hygiene_vs_issue_report(self, across_type, create_dataaccuracy_object, create_generic_object):
        ...
        for val in df_sprint:
            try:
                ...
                issue_payload = create_dataaccuracy_object.accuracy_payload(
                    ...
                    across=across_type,
                    sort_value=[{"id": "ticket_count", "desc": True}], ou_ids=val, customfield=customfield,
                ...
                validation_dropdown = create_dataaccuracy_object.dropdown_validation(hygeine_response, across_type, val,
                                                                                     ou_user_filter, int_ids)
    ...
    @pytest.mark.parametrize("across_type", across_type)
    def test_comparing_dashboard_filter_hygiene_vs_issue_report(self, across_type, create_dataaccuracy_object,
    ...
    @pytest.mark.dataaccuracy
    @pytest.mark.parametrize("across_type", across_type)
    def test_comparing_hygiene_vs_issue_report_with_exclude(self, across_type, create_dataaccuracy_object,
                                                            create_generic_object):
        df_sprint = create_generic_object.env["set_ous"]
        hygiene_types = create_generic_object.api_data['hygiene_types']
        ou_user_filter = create_generic_object.env["ou_user_filter_designation"]
