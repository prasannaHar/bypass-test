...
class TestScmFirstReviewTrendsReportsCustomer:
    generic_object = TGhelper()
    across_type = generic_object.api_data["scm-prs-first_review-trends-report-across"]
    ...
    def test_scm_prs_first_review_trends_report(self, across_type, create_generic_object,
        ...
        for val in df_sprint:
            try:
                ...
                es_base_url = create_generic_object.connection[
                                  ...
                                  "scm-prs-first_review-trends-report"] + "?there_is_no_cache=true&force_source=es"
                ...
                db_base_url = create_generic_object.connection[
                                  "base_url"] + create_generic_object.api_data[
                                  "scm-prs-first_review-trends-report"] + "?there_is_no_cache=true&force_source=db"
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db(es_response, db_response,
                                                                            columns=['key', 'additional_key', 'median',
                                                                                     'min', 'max', 'count',
                                                                                     'sum'], add_keys=True, unique_id="filter with ou id: {}".format(str(across_type)+" "+ str(val)))
                if not flag:
    ...
    @pytest.mark.esvsdbscm
    def test_scm_prs_first_review_trends_single_stat_report(self, create_generic_object,
                                                            create_customer_object, get_integration_obj):
        ...
        for val in df_sprint:
            for each_agg_type in agg_type:
                try:
                    filters = {"agg_type": each_agg_type, "integration_ids": get_integration_obj}
                    ...
                    flag, zero_flag = create_customer_object.comparing_es_vs_db(es_response, db_response,
                                                                                columns=['key', 'additional_key',
                                                                                         'median',
                                                                                         'min', 'max', 'count',
                                                                                         'sum'], add_keys=True, unique_id="ou_id is :"+val)
                    if not flag:
                        list_not_match.append(val)
                    if not zero_flag:
                        zero_list.append(val)
    ...
    def test_scm_prs_first_review_trends_report_es_filter(self, filter_type, create_generic_object,
                                                          create_customer_object, get_integration_obj):
        ...
        for val in df_sprint:
            try:
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db(es_response, db_response,
                                                                            columns=['key', 'additional_key', 'median',
                                                                                     'min', 'max', 'count',
                                                                                     'sum'], add_keys=True, unique_id="filter with ou id: {}".format(str(filter_type)+" "+ str(val)))
                if not flag:
                    list_not_match.append(val)
                if not zero_flag:
                    zero_list.append(val)
    ...
    @pytest.mark.parametrize("filter_type", filter_type)
    def test_scm_prs_first_review_trends_report_es_excludefilter(self, filter_type, create_generic_object,
                                                                 create_customer_object, get_integration_obj):
    ...
    def test_scm_prs_first_review_trends_report_es_partialmatchfilter(self, create_generic_object,
                                                                      create_customer_object, get_integration_obj):
    ...
    @pytest.mark.parametrize("filter_type", filter_type)
    def test_scm_prs_first_review_trends_single_stat_report_es_filter(self, filter_type, create_generic_object,
                                                                      create_customer_object, get_integration_obj):
    ...
    @pytest.mark.parametrize("filter_type", filter_type)
    def test_scm_prs_first_review_trends_single_stat_report_es_excludefilter(self, filter_type, create_generic_object,
    ...
    def test_scm_prs_first_review_trends_single_stat_report_es_partialmatchfilter(self, create_generic_object,
                                                                                  create_customer_object,
                                                                                  get_integration_obj):
