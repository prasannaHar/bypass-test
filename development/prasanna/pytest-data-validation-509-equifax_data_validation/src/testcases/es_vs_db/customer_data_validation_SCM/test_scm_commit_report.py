...
class TestSCMCommitsreport:
    ...
    def test_scm_commits_report(self, agg_type, create_generic_object,
        ...
        for val in df_sprint:
            try:
                ...
                payload = {"filter": filters, "across": agg_type,
                           "ou_ids": [val]}
                ...
                es_base_url = create_generic_object.connection[
                                  ...
                                  "scm-commit-single-stat"] + "?there_is_no_cache=true&force_source=es"
                ...
                db_base_url = create_generic_object.connection[
                                  "base_url"] + create_generic_object.api_data[
                                  "scm-commit-single-stat"] + "?there_is_no_cache=true&force_source=db"
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,
                                                                                           ...
                                                                                           columns=['key',
                                                                                                    ...
                                                                                                    'pct_legacy_refactored_lines'], unique_id="filter with ou id: {}".format(str(agg_type)+" "+ str(val)))
                if not flag:
    ...
    @pytest.mark.parametrize("trends_type", trends_type)
    def test_scm_commits_report_trends(self, trends_type, create_generic_object,
                                       create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
        zero_list = []
        list_not_match = []
        ...
        for val in df_sprint:
            try:
                ...
                payload = {"filter": filters, "across": "trends", "interval": trends_type,
                           "ou_ids": [val]}
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,
                                                                                           ...
                                                                                           columns=['key',
                                                                                                    ...
                                                                                                    "pct_new_lines",
                                                                                                    "pct_refactored_lines",
                                                                                                    "pct_legacy_refactored_lines"], unique_id="filter with ou id: {}".format(str(trends_type)+" "+ str(val)))
                if not flag:
                    list_not_match.append(val)
                if not zero_flag:
                    zero_list.append(val)
    ...
    def test_scm_commits_report_es_filter(self, filter_type, create_generic_object,
                                          create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
        zero_list = []
        list_not_match = []
            ...
            try:
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,
                                                                                           ...
                                                                                           columns=['key',
                                                                                                    ...
                                                                                                    'pct_new_lines',
                                                                                                    'pct_refactored_lines',
                                                                                                    'pct_legacy_refactored_lines'], unique_id="filter with ou id: {}".format(str(filter_type)+" "+ str(val)))
                if not flag:
                    list_not_match.append(val)
                if not zero_flag:
                    zero_list.append(val)
    ...
    @pytest.mark.parametrize("filter_type", filter_type)
    def test_scm_commits_report_es_excludefilter(self, filter_type, create_generic_object,
                                                 create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
        zero_list = []
        list_not_match = []
    ...
    def test_scm_commits_report_es_partialmatchfilter(self, create_generic_object,
                                                      create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
        zero_list = []
        list_not_match = []
            ...
            try:
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,
                                                                                           ...
                                                                                           columns=['key',
                                                                                                    ...
                                                                                                    'pct_new_lines',
                                                                                                    'pct_refactored_lines',
                                                                                                    'pct_legacy_refactored_lines'], unique_id="ou_id is :"+val)
                if not flag:
                    list_not_match.append(val)
                if not zero_flag:
                    zero_list.append(val)
