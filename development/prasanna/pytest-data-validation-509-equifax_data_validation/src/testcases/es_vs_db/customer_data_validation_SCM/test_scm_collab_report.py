...
class TestSCMCollabCustomer:
    generic_object = TGhelper()
    filter_type = generic_object.api_data["scm_prs_report_filter_types"]

    @pytest.mark.esvsdbscm
    @pytest.mark.run(order=1)
    def test_scm_collab_report_es(self, create_generic_object,
                                  create_customer_object, get_integration_obj):
        pr_creators = create_generic_object.get_filter_options_scm(arg_filter_type="creator")
        ...
        for val in df_sprint:
            try:
                ...
                es_base_url = create_generic_object.connection[
                                  ...
                                  "scm_collab_report"] + "?there_is_no_cache=true&force_source=es"
                ...
                db_base_url = create_generic_object.connection[
                                  "base_url"] + create_generic_object.api_data[
                                  "scm_collab_report"] + "?there_is_no_cache=true&force_source=db"
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,
                                                                                           sort_column_name=['key',
                                                                                                             "collab_state"],
                                                                                           sort_values_inside_columns_dict=[
                                                                                               'stacks'],
                                                                                           columns=['key',
                                                                                                    'additional_key',
                                                                                                    'count',
                                                                                                    "collab_state",
                                                                                                    "stacks"], unique_id="ou_id is :"+val)
                if not flag:
    ...
    def test_scm_collab_report_es_filter(self, filter_type, create_generic_object,
                                         create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
        zero_list = []
        list_not_match = []
            ...
            try:
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,
                                                                                           ...
                                                                                           columns=['key',
                                                                                                    ...
                                                                                                    'count',
                                                                                                    "collab_state",
                                                                                                    "stacks"], unique_id="filter with ou id: {}".format(str(filter_type)+" "+ str(val)))
                if not flag:
                    list_not_match.append(val)
                if not zero_flag:
                    zero_list.append(val)
    ...
    @pytest.mark.parametrize("filter_type", filter_type)
    def test_scm_collab_report_es_excludefilter(self, filter_type, create_generic_object,
                                                create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
        zero_list = []
        list_not_match = []
    ...
    def test_scm_collab_report_es_partialmatchfilter(self, create_generic_object,
                                                     create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
        zero_list = []
        list_not_match = []
        ...
        for val in df_sprint:
            try:
                filters = {"missing_fields": {"pr_merged": True}, "pr_closed_at": {"$gt": gt, "$lt": lt},
                           "integration_ids": get_integration_obj,
