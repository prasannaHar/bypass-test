...
class TestIssuesCustomer:
    ...
    def test_sprint_metrics_single_stat_report_jira_es(self,  create_generic_object,
        ...
        for val in df_sprint:
            try:
                filters = {"include_issue_keys":True,
                            "agg_type":"average",
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,sort_column_name='sprint_id',
                                                                                columns=['key', 'additional_key',
                                                                                         ...
                                                                                         "total_issues","total_unestimated_issues"], unique_id="ou_id is :"+val)
                if not flag:
    ...
    def test_sprint_metrics_single_stat_report_jira_filter_es(self, filter_type, create_generic_object,
                                                              create_customer_object, get_integration_obj):
    ...
    @pytest.mark.esdbfilter
    @pytest.mark.parametrize("filter_type", filter_type)
    def test_sprint_metrics_single_stat_report_jira_exclude_filter_es(self, filter_type, create_generic_object,
                                                                      create_customer_object, get_integration_obj):
    ...
    def test_sprint_metrics_single_stat_report_jira_dashboard_partial_filter_es(self, dashboard_partial,
    ...
    def test_sprint_metrics_single_stat_report_jira_es_for_months(self, interval, create_generic_object,
                                                                  create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
        zero_list = []
        list_not_match = []
        not_executed_list = []
        gt, lt = create_generic_object.get_epoc_time(value=interval)

        get_filter_response = create_generic_object.get_filter_options(arg_filter_type=["issue_type"],
                                                                       arg_integration_ids=get_integration_obj)
        if len(get_filter_response['records'][0]["issue_type"]) == 0:
            pytest.skip("Filter Doesnot have Any values")
        all_filter_records = [get_filter_response['records'][0]["issue_type"]]
        value = []

        ran_value = random.sample(all_filter_records[0], min(2, len(all_filter_records[0])))
            ...
            try:
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,
                                                                                           ...
                                                                                           columns=['key',
                                                                                                    ...
                                                                                                    "delivered_creep_keys",
                                                                                                    "total_issues",
                                                                                                    "total_unestimated_issues"], unique_id="interval with ou id: {}".format(str(interval)+" "+ str(val)))
                if not flag:
                    list_not_match.append(val)
                if not zero_flag:
                    zero_list.append(val)
