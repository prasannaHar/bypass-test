...
class TestIssuesCustomer:
    ...
    def test_sprint_goal_report_jira_es(self,  create_generic_object,
        ...
        for val in df_sprint:
            try:
                filters = {"completed_at":{"$gt": gt, "$lt": lt},"integration_ids": get_integration_obj}
                ...
                es_base_url = create_generic_object.connection[
                                  ...
                                  "sprint_goal_report"] + "?there_is_no_cache=true&force_source=es"
                ...
                db_base_url = create_generic_object.connection[
                                  "base_url"] + create_generic_object.api_data[
                                  "sprint_goal_report"] + "?there_is_no_cache=true&force_source=db"
                db_response = create_generic_object.execute_api_call(db_base_url, "post", data=payload)
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,sort_column_name='id',
                                                                                columns=['id','sprint_id','name','state','goal',
                                                                                'start_date','end_date','complete_date'], unique_id="ou_id is :"+val)
                if not flag:
    ...
    def test_sprint_goal_report_jira_filters_es(self, filter_type, create_generic_object,
                                                create_customer_object, get_integration_obj):
        ...
        for val in df_sprint:
            try:
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,
                                                                                           sort_column_name='id',
                                                                                           columns=['id', 'sprint_id',
                                                                                                    'name', 'state',
                                                                                                    'goal',
                                                                                                    'start_date',
                                                                                                    'end_date',
                                                                                                    'complete_date'], unique_id="filter with ou id: {}".format(str(filter_type)+" "+ str(val)))
                if not flag:
                    list_not_match.append(val)
                if not zero_flag:
                    zero_list.append(val)
    ...
    @pytest.mark.esdbfilter
    @pytest.mark.parametrize("filter_type", filter_type)
    def test_sprint_goal_report_jira_exclude_filters_es(self, filter_type, create_generic_object,
                                                        create_customer_object, get_integration_obj):
    ...
    def test_sprint_goal_report_jira_dashboard_partial_filters_es(self, dashboard_partial, create_generic_object,
                                                                  create_customer_object, get_integration_obj):
            ...
            try:
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db_without_prefix(es_response, db_response,
                                                                                           ...
                                                                                           columns=['id', 'sprint_id',
                                                                                                    ...
                                                                                                    'start_date',
                                                                                                    'end_date',
                                                                                                    'complete_date'], unique_id="dashboard_partial with ou id: {}".format(str(dashboard_partial)+" "+ str(val)))
                if not flag:
                    list_not_match.append(val)
                if not zero_flag:
                    zero_list.append(val)
