...
class TestIssuesCustomer:
    ...
    hygiene_types = generic_object.api_data["hygiene_types"]
    ...
    def test_issue_hygiene_trend_es(self, hygiene_types, create_generic_object,
        ...
        list_not_match = []
        not_executed_list = []
        gt_created, lt_created = create_generic_object.get_epoc_time(value=12)
        for val in df_sprint:

            try:
                filters = {"visualization": "stacked_area", "projects": create_generic_object.env["project_names"], "partial_match": {},
                           "issue_created_at": {"$gt": gt_created, "$lt": lt_created}, "product_id": create_generic_object.env["project_id"],
                           "integration_ids": get_integration_obj, "hygiene_types": [hygiene_types]}

                payload = {"filter": filters, "across": "trend", "page_size": 1, "page": 0, "interval": "month",
                           "ou_ids": [val],
                           }
                ...
                flag, zero_flag = create_customer_object.comparing_es_vs_db(es_response, db_response,
                                                                            columns=['key', 'additional_key',
                                                                                     'mean_story_points',
                                                                                     'total_tickets',
                                                                                     'total_story_points'], unique_id="hygiene_types with ou id: {}".format(str(hygiene_types)+" "+ str(val)))
    ...
    def test_issue_hygiene_trend_es_filters(self, filter_type, create_generic_object,
                                            create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
        ...
        for val in df_sprint:
            try:
                ...
                filters = {"visualization": "stacked_area",
                           "integration_ids": get_integration_obj, "hygiene_types": ["IDLE"]}
    ...
    @pytest.mark.esdbfilter
    @pytest.mark.parametrize("filter_type", filter_type)
    def test_issue_hygiene_trend_es_exclude_filters(self, filter_type, create_generic_object,
                                                    create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
    ...
    def test_issue_hygiene_trend_es_dashboard_partial_filters(self, dashboard_partial, create_generic_object,
                                                              create_customer_object, get_integration_obj):
        df_sprint = create_generic_object.env["set_ous"]
