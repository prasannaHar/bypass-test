...
class TestAggsSCMDataValidation:
    ...
    def test_aggs_testing_scm_commit_table(self,scm_table, create_generic_object, create_postgres_object,
                                           create_customer_object, create_sql_query_object_gitlab):
        ...
        for each_int_map in scm_int_map:
            ...
            try:
                ...
                if scm_table in ["scm_pullrequest_labels"]:
                    new_flow_query = create_sql_query_object_gitlab.scm_pullrequest_labels_query(integration_id=new_flow_id,
                                                                            tenant_name=tenant_name)
                    old_flow_query = create_sql_query_object_gitlab.scm_pullrequest_labels_query(integration_id=existing_flow_id,
                                                                            tenant_name=tenant_name)
                if scm_table in ["scm_pullrequest_reviews"]:
                    new_flow_query = create_sql_query_object_gitlab.scm_pullrequest_reviews_query(integration_id=new_flow_id,
                                                                             tenant_name=tenant_name)
                    old_flow_query = create_sql_query_object_gitlab.scm_pullrequest_reviews_query(integration_id=existing_flow_id,
                                                                             tenant_name=tenant_name)
                if scm_table in ["scm_file_commits"]:
                    new_flow_query = create_sql_query_object_gitlab.scm_file_commits_query(integration_id=new_flow_id,
                                                                      tenant_name=tenant_name)
                    old_flow_query = create_sql_query_object_gitlab.scm_file_commits_query(integration_id=existing_flow_id,
                                                                      tenant_name=tenant_name)



                if scm_table in ['github_project_columns']:
                    new_flow_query = create_sql_query_object_gitlab.scm_github_project_cols_query(tenant_name=tenant_name,
                                                                             integration_id=new_flow_id)

                    old_flow_query = create_sql_query_object_gitlab.scm_github_project_cols_query(tenant_name=tenant_name,
                                                                             integration_id=existing_flow_id)

                if scm_table in ["github_cards"]:
                    new_flow_query = create_sql_query_object_gitlab.github_cards_query(tenant_name=tenant_name,
                                                                  integration_id=new_flow_id)

                    old_flow_query = create_sql_query_object_gitlab.github_cards_query(tenant_name=tenant_name,
                                                                  integration_id=existing_flow_id)
                ...
                if scm_table in ['scm_commits', 'scm_files', 'scm_pullrequests', 'scm_pullrequests_jira_mappings',
                    ...
                    if scm_table not in ['scm_pullrequests_jira_mappings', 'scm_pullrequests_workitem_mappings',
                        ...
                        new_flow_query = create_sql_query_object_gitlab.star_query(table_name=table_name, integration_id=new_flow_id)
                        old_flow_query = create_sql_query_object_gitlab.star_query(table_name=table_name, integration_id=existing_flow_id)
                    elif scm_table in ["scm_commit_jira_mappings"]:
                        new_flow_query = create_sql_query_object_gitlab.star_query(table_name=table_name, scm_integ_id=new_flow_id)
                        old_flow_query = create_sql_query_object_gitlab.star_query(table_name=table_name, scm_integ_id=existing_flow_id)

                    elif scm_table in ['gitrepositories']:

                        new_flow_query = create_sql_query_object_gitlab.scm_gitrepositories_query(table_name=table_name,
                                                                             integration_id=new_flow_id)
                        old_flow_query = create_sql_query_object_gitlab.scm_gitrepositories_query(table_name=table_name,
                                                                             integration_id=existing_flow_id)

                    else:
                        new_flow_query = create_sql_query_object_gitlab.star_query(table_name=table_name, scm_integration_id=new_flow_id)
                        old_flow_query = create_sql_query_object_gitlab.star_query(table_name=table_name,
    ...
    def test_scm_commit_pullrequest_mappings(self, create_postgres_object, create_sql_query_object_gitlab,
        ...
        try:
            for each_int_map in scm_int_map:
                ...
                query = create_sql_query_object_gitlab.scm_commit_pullrequest_mappings_except_query(tenant_name=tenant_name,
        ...
        assert len(no_data_list) == 0, "Integrations mapping with no data - list is {}".format(
            set(mismatch_data_list))
        assert len(mismatch_data_list) == 0, "Integrations mapping with mismatch data - list is {}".format(
            set(mismatch_data_list))
