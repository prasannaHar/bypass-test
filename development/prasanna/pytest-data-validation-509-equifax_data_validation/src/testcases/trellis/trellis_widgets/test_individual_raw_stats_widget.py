...
class TestIndividualRawStatsReport:
    ...
    # org_admin_flg = generic_object.env.get("org_admin_feature")
    # if not org_admin_flg:
    #     rbacusertype = [x for x in rbacusertype if x not in [ "ORGADMIN_TRELLIS", "ORGADMIN", "ADMIN_TRELLIS_OUs"]]
    # else:
    #     rbacusertype = [x for x in rbacusertype if x not in ["PUBLIC_DASHBOARD_Manager", "ADMIN_Manager"]]

    @pytest.mark.trellisrbac
    @pytest.mark.run(order=1)
    @pytest.mark.parametrize("rbacusertype", rbacusertype)
    def test_individual_raw_stats_report_tc001_widget_creation_rbac(self, create_widget_helper_object,rbacusertype,
                                                               create_generic_object, create_customer_object,ou_helper_object):
        """Validate individual raw stats widget functionality"""
        org_ids = create_generic_object.env["set_trellis_ous"]
        not_executed_list = []
        for org_id in org_ids:
            try:
                filters = {"ou_ref_ids": [org_id], "interval": "LAST_MONTH"}
                response_data = create_widget_helper_object.create_individual_raw_stats(filters=filters, user_type=rbacusertype)
                executed_flag = True
    ...
    def test_individual_raw_stats_report_tc001_widget_creation(self, create_widget_helper_object,req_interval,
    ...
    @pytest.mark.parametrize("req_interval", req_interval)
    def test_individual_raw_stats_report_tc002_invalid_values_check(self, create_widget_helper_object,req_interval,
                                                                    create_generic_object, create_customer_object):
    ...
    @pytest.mark.parametrize("req_interval", req_interval)
    def test_individual_raw_stats_report_tc003_ou_associated_users_check(self, create_widget_helper_object, ou_helper_object,
                                                                    create_generic_object, create_customer_object, req_interval):
        """Validate individual raw stats widget functionality - negative values check"""
        org_ids = create_generic_object.env["set_trellis_ous"]
        not_executed_list = []
        invalid_ou_data_list = []
