...
class TestRawStatsByOrgUnitsReport:
    ...
    def test_raw_stats_by_org_units_report_tc001_widget_creation_rbac(self, create_widget_helper_object, rbacusertype,
                                                                 create_generic_object, create_customer_object, ou_helper_object):
        """Validate raw stats by org units widget functionality invalid values check"""
        org_ids = create_generic_object.env["set_trellis_ous"]
        not_executed_list = []
        for org_id in org_ids:
            try:
                filters = {"ou_ref_ids": [org_id], "interval": "LAST_MONTH"}
                response_data = create_widget_helper_object.create_raw_stats_by_org_units(filters=filters, user_type=rbacusertype)
                executed_flag = True
    ...
    def test_raw_stats_by_org_units_report_tc001_widget_creation(self, create_widget_helper_object,req_interval,
        ...
        for org_id in org_ids:
            try:
                ...
                response_data = create_widget_helper_object.create_raw_stats_by_org_units(filters=filters)
                executed_flag, valid_data_flag = create_customer_object.trellis_data_validator(response=response_data,
                                                                                               raw_stats=True)
                if not executed_flag:
    ...
    @pytest.mark.parametrize("req_interval", req_interval)
    def test_raw_stats_by_org_units_report_tc002_invalid_values_check(self, create_widget_helper_object,req_interval,
                                                                      create_generic_object, create_customer_object):
        """Validate raw stats by org units widget functionality invalid values check"""
        org_ids = create_generic_object.env["set_trellis_ous"]
        not_executed_list = []
    ...
    @pytest.mark.parametrize("req_interval", req_interval)
    def test_raw_stats_by_org_units_report_tc003_ou_and_associated_ou_scores_check(self, create_widget_helper_object,
                                                                                   create_generic_object,
                                                                                   create_customer_object,
                                                                                   ou_helper_object, req_interval):
        """Validate raw stats by org units widget functionality invalid values check"""
        org_ids = create_generic_object.env["set_trellis_ous"]
        not_executed_list = []
        invalid_ou_data_list = []
