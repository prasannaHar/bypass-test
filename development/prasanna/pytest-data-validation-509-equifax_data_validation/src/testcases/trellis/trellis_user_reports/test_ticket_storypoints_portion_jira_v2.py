...
class TestTrellisTicketStoryPointPortion:
    ...
    def test_trellis_ticket_and_storypoint_assigneetime_portion(self,jira_feature, create_widget_helper_object,
                                    get_integration_obj,create_generic_object, create_customer_object):
        

        status_mapping = pd.read_csv("./testcases/trellis_user_reports/jira_statuses.csv")
        in_progress_status_df = status_mapping[status_mapping["StatusCategory"] == "IN PROGRESS"]
        in_progress_status_df_copy = deepcopy(in_progress_status_df)
        ...
        for eachrecord in required_tests:
            try:
                ...
                ## Development stages retrival
                trellis_profile_id = userid_and_profile_id[1]
                profile_settings = create_widget_helper_object.trellis_profiles_settings_retriever(profile_id=trellis_profile_id)
                development_stages = (profile_settings["settings"])["development_stages"]
                if len(development_stages)>0:
                    in_progress_status_df = in_progress_status_df_copy[in_progress_status_df_copy["Status"].isin(development_stages)]
                in_progress_status_ids = in_progress_status_df["Id"].values.tolist()
                ...
                for eachissue in issues:
                    ...
                    assignee_time_actual = int(eachissue["assignee_time"]/1000)
                    ...
                    story_points, ticket_portion_calc, storypoints_portion_calc, assignee_time_calc = create_customer_object.\
                        trellis_calculate_ado_ticket_portion_v2(workitem_response=issue_response, 
                                                             ...
                                                             status_ids=in_progress_status_ids)
                    ...
                    assignee_time_diff = assignee_time_actual - assignee_time_calc
                    ...
                    if not ( -0.05 <= storypoints_result_diff <= 0.05):
                        ...
                        failure_flag = True
                    if not ( -10 <= assignee_time_diff <= 10):
                        list_not_match.append(
                            "Assignee time difference: org user details {orguserid} \
                                & workitem id - {id} with difference {res_diff} ".format(
                            orguserid=eachrecord, id=issue_keys, res_diff=assignee_time_diff))
                        failure_flag = True
                    if(failure_flag):
                        failure_data.append([userid_and_profile_id, issue_keys[0],story_points,
                                    ...
                                    assignee_time_actual,assignee_time_calc ,assignee_time_diff])
        ...
        if len(failure_data)>=1:
            failure_data_df = pd.DataFrame(failure_data, columns = ["org user name", "workitem id", "story points",
                                ...
                                "assignee time-feature list", "assignee time-workitem list", "assignee-time-diff" ])
