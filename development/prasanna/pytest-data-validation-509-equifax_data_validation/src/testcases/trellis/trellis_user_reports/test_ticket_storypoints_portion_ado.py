...
class TestTrellisTicketStoryPointPortion:
    ...
    @pytest.mark.parametrize("ado_feature", ado_feature)
    @pytest.mark.run(order=1)
    def test_trellis_ticket_and_storypoint_portion(self,ado_feature, create_widget_helper_object,
            ...
            try:
                ...
                base_url = create_generic_object.connection["base_url"] + \
                    create_generic_object.api_data["trellis_feature_list"] + \
                        "?there_is_no_cache=true&force_source=es"
                feature_list_response = create_generic_object.execute_api_call(base_url, "post", data=payload)
                workitems = (feature_list_response["records"][0])["records"]
                ...
                for eachworkitem in workitems:
                    failure_flag = False
                    workitem_ids = [eachworkitem["workitem_id"]]
                    assignee_name = userid_and_profile_id[2]
                    ticket_portion_actual = eachworkitem["ticket_portion"]
                    story_points_portion_actual = eachworkitem["story_points_portion"]

                    workitem_response = create_widget_helper_object.retrieve_workitem_details(
                        integration_id=get_integration_obj, workitem_ids=workitem_ids)
                    story_points, ticket_portion_calc, storypoints_portion_calc = create_customer_object.trellis_calculate_ado_ticket_portion(
                                                            workitem_response=workitem_response, assignee_name=assignee_name)
                    ...
                    if not ( -0.05 <= tickets_result_diff <= 0.05):
                        list_not_match.append(
                            ...
                            orguserid=eachrecord, id=workitem_ids, res_diff=tickets_result_diff))
                        failure_flag = True
                    if not ( -0.05 <= storypoints_result_diff <= 0.05):
                        list_not_match.append(
                            "Story points portion difference: org user details {orguserid} \
                                & workitem id - {id} with difference {res_diff} ".format(
                            orguserid=eachrecord, id=workitem_ids, res_diff=storypoints_result_diff))
                        failure_flag = True
                    if(failure_flag):
                        failure_data.append([userid_and_profile_id, workitem_ids[0],story_points,
        ...
        if len(failure_data)>=1:
            ...
            failure_data_df.to_csv("log_updates/trellis_story_ticket_portion_result" + ado_feature + ".csv")
